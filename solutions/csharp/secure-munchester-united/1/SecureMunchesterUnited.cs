public class SecurityPassMaker
{
    ...
    {
        string displayName = "Too Important for a Security Pass";
        
        if (support is Staff staff) {
            bool isSecurityTeam = staff.GetType() == typeof(Security);
            displayName = $"{staff.Title}{(isSecurityTeam ? " Priority Personnel" : String.Empty)}";
        }
